...
function Encoder (opts) {
  ...
  if(opts.float && opts.bitDepth == 64) {
    this.inputType = PCM_TYPE_DOUBLE;
  } else if(opts.float && opts.bitDepth == 32) {
    this.inputType = PCM_TYPE_FLOAT;
  } else {
    this.inputType = PCM_TYPE_SHORT_INT; //default is for short int
  }
...
Object.defineProperty(Encoder.prototype, 'bitDepth', {
  ...
  set: function (v) {
    return this._bitDepth = v;
  }
});

/**
 * The lame encoder only supports "signed" data types. See the note for
 * "bitDepth" above.
 */
