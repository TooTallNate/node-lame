...
Encoder.prototype._process = function _process () {
  ...
  if (flushing) {
    ...
    debug('invoke async encode flush');
    ...
    debug('invoke async encode:', chunk.length, num, this._offset, availOutBefore)
  ...
  function afterWrite (bytesWritten) {
    debug('afterWrite:', bytesWritten)
    if (bytesWritten < 0) {
      var err = new Error(ENCODE_ERRORS[bytesWritten]);
