...
binding.mpg123_init();
process.on('exit', binding.mpg123_exit);
...
Decoder.prototype._transform = function (chunk, write, done) {
  ...
  function afterFeed (ret) {
    chunk; // XXX: a hack to ensure that "chunk" doesn't get GC'd until after
           // feed() is done. We could do this in C++-land to be "clean", but
           // doing this saves sizeof(Persistent<Object>) from the req struct.
    debug('mpg123_feed():', ret);
    if (MPG123_OK != ret) {
  ...
  function afterRead (ret, bytes) {
    debug('mpg123_read():', ret, bytes);
    if (bytes > 0) {
      // got decoded data
      assert(out.length >= bytes);
      if (out.length != bytes) {
        debug('slicing from %d to %d', out.length, bytes)
