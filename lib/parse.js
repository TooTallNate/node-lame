...
exports.parseFrameHeader = function parseFrameHeader (b) {
  ...
  //console.error(b.length, b, b.toString());
  var r = {};
  var header32 = b.readUInt32BE(0);
  ...
  if (r.layerID === LAYER1_ID) {
    ...
    r.bytesPerSlot = 4;
    //r.frameSize = (12 * (r.bitrateKBPS*1000) / (r.samplingRateHz*10) + (r.padding ? 1 : 0)) * 4;
  } else {
    ...
    r.bytesPerSlot = 1;
    //r.frameSize = ~~(144 * (r.bitrateKBPS*1000) / r.samplingRateHz + (r.padding ? 1 : 0));
  }
  r.frameSizeRaw = ((r.bitrateKBPS * 125) * r.samplesPerFrame) / r.samplingRateHz + (r.padding ? r.bytesPerSlot : 0);
  r.frameSize = ~~r.frameSizeRaw;
  if (!r.frameSize) throw new Error('bad size: ' + r.frameSize);
